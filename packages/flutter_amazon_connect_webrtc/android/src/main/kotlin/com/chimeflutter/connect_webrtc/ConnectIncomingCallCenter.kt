 ...
 * when the FCM data push (`type == "incomingConnectCall"`) arrives — even when no Flutter engine is
 ...
 *
 * When the user answers, the event is forwarded to the Flutter side (`incomingCallAnswered` on the
 * plugin event channel) — or parked until the engine attaches (cold start) and drained via the
 * `getPendingIncomingCall` method channel call. Declines that cannot reach Dart are recovered by
...
object ConnectIncomingCallCenter {
    ...
    /** Set by the plugin while a Flutter engine is attached; events flow to the Dart event channel. */
    @Volatile
    ...
    /** Dismisses a still-ringing call (caller cancelled / answered elsewhere). Plugin-facing. */
    @JvmStatic
    @Synchronized
    ...
    /** Drains the parked cold-start answer (plugin `getPendingIncomingCall`). */
    @Synchronized
    ...
    internal fun onAnswerAction(context: Context) {
        ...
        // Bring the host app to the foreground so the Flutter side can join the call.
    ...
    private fun dispatch(event: Map<String, Any?>) {
        ...
        if (active != null) {
            ...
            // Cold start: the engine is not running yet — park the answer for pickup at startup.
