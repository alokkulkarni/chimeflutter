...
/// PushKit delegate *before any Flutter engine exists* (Apple requires `reportNewIncomingCall`
/// synchronously for every VoIP push):
///
/// ```swift
...
/// ```
///
/// When the user answers, the answer is forwarded to Dart (event `incomingCallAnswered`) — or
/// parked in ``pendingAnsweredCall`` until the Flutter side attaches (cold start) and drains it via
...
public final class ConnectCallKitManager: NSObject, CXProviderDelegate {
    ...
    /// Attached by the plugin at registration; nil while only the ring UI is active (cold start).
    weak var chime: ChimeCallManager?
    /// Plugin event bridge (marshals onto the Flutter event channel).
    ...
    /// The answer that happened before Flutter attached; drained by `getPendingIncomingCall`.
    ...
    /// Drains the parked cold-start answer (called by the plugin's `getPendingIncomingCall`).
    ...
    private func notify(_ event: [String: Any]) {
        if let listener = eventListener {
            ...
            // Flutter is not attached yet (cold start) — park the answer for pickup at startup.
    ...
    public func provider(_ provider: CXProvider, perform action: CXAnswerCallAction) {
        ...
        // Dart now exchanges the callId for join credentials and joins with asIncoming=true; the
    ...
    public func provider(_ provider: CXProvider, perform action: CXEndCallAction) {
        if let callId = incomingCallId, !incomingAnswered {
            // Declined from the ring UI: no media exists yet — just notify so Dart tells the
