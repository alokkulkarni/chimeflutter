...
/**
 * Adapts the backend `CallSession` map (sent by the JS controller) into a Chime
 * [MeetingSessionConfiguration].
 *
 * The load-bearing native transform and the one piece unit tested (pure — no Android deps). Mirrors
 * AWS's official Android sample: read `meeting`/`attendee` (the backend already unwrapped Connect's
